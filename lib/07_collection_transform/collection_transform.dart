...
void main() {
  final currentDay = DateTime(2022, 12, 17);

  final Categories itog = {};

  final listProducts = data
      .where((element) =>
          element.qty > 0 && element.expirationDate.isAfter(currentDay))
      .toList();

  for (var product in listProducts) {
    if (!itog.containsKey(product.categoryName)) {
      itog[product.categoryName] = {};
    }
    if (!itog[product.categoryName]!.containsKey(product.subcategoryName)) {
      itog[product.categoryName]?[product.subcategoryName] = [];
    }
    itog[product.categoryName]?[product.subcategoryName]?.add(product.name);
  }
  print(itog);
}
